...
import type { Message, ContextConfig, ContextState } from '../types';
...
export class ContextManager {
  ...
  constructor(config: Partial<ContextConfig> = {}) {
    ...
    // 简单的 token 估算器
    this.tokenEstimator = (text: string) => {
   ...
   * 压缩上下文
   */
  async compress(summarizer: (messages: Message[]) => Promise<string>): Promise<void> {
    ...
    // 重新计算 token 数
    this.state.tokenCount = this.state.messages.reduce(
   ...
   * 注入记忆
   */
  injectMemory(memoryContent: string): void {
    if (this.state.memoryInjected) return;
